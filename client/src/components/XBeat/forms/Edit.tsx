...
const EditBeat = ({ visible, setVisible }: Props) => {
  const [t] = useTranslation('global')
  const dispatch = useAppDispatch()
  const activeEditingBeat = useAppSelector((state) => state?.client?.beats?.activeEditingBeat)
            ...
            <div className='flex w-full flex-col gap-5 overflow-y-hidden'>
              <div className='flex flex-col items-center justify-center gap-0'>
                <h4 className='text-titulo3-regular text-center'>
                  {t('editBeat.t1')} <span className='text-titulo3-semibold text-red-700'>beat</span>{' '}
                </h4>
                <p className='text-base-light text-center'>{t('editBeat.t2')}</p>
              </div>
