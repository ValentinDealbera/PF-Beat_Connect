...
export default function BeatFilters() {
  ...
  const { genres, genresFilter: genre } = useAppSelector((state) => state.filters)
        ...
        <>
          <div className='z-10 hidden flex-row justify-between sm:flex'>
            <div className='gap-estilo3 flex items-end'>
              ...
              />
              <ModalMinMax label='BPM' seleccionados={BPM} setSeleccionados={setBPM} />
            </div>
            <Select
              value={sorter}
            ...
            {dropDownFilter && (
              <>
                ...
                >
                      ...
                      dynamicFilterBtns={[
                        {
                          ...
                          handleClick: () => {
                            setChildFilterIndex(1)
                          }
                        },
                        {
                          label: i18n.language === 'en' ? 'Price' : 'Precio',
                          handleClick: () => {
                            setChildFilterIndex(2)
                          }
                        },
                        {
                          label: 'BPM',
                          handleClick: () => {
                            setChildFilterIndex(3)
                          }
                        }
                      ]}
                    />
                  ...
                  {childFilterIndex === 3 && <MinMax label='BPM' seleccionados={BPM} setSeleccionados={setBPM} />}
                </BeatBottomSheetFilteringWithHeader>
              </>
            )}
            <div>
...
interface SelectProps {
  valores: Array<{ value: string; label: string }>
