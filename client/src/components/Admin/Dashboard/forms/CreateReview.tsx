...
const AdminCreateReviewForm = forwardRef((props: Props, ref) => {
  ...
  const defaultValues = useAppSelector((state) => state?.admin?.reviews?.currentEditingReview) || {}
  ...
  const onSubmit = async () => {
    const actionToDispatch = mode === 'edit' ? adminEditReview : adminPostReview
    try {
      await handleSubmit({
        form,
        actionToDispatch,
      ...
      <FormContainer>
        <FormRow>
          <FormColumn className='w-full'>
            {mode === 'create' && (
              <label htmlFor='createdBy' className='text-sm-medium flex min-w-0 flex-col gap-1'>
                {' '}
          ...
          <FormColumn className='w-full'>
            {mode === 'create' && (
              <label htmlFor='beat' className='text-sm-medium flex min-w-0 flex-col gap-1'>
                {' '}
                Beat
                <Autocomplete
                  id='beat'
