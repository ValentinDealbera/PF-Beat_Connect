    ...
    switch (field) {
      case 'username':
        if (form.username.length < 1 || form.username.length > 20 || !form.username) {
          error.username = 'Your username must have between one and twenty characters.'
        }
      ...
      case 'lastName':
        if (form.lastName.length < 1 || form.lastName.length > 50 || !form.lastName) {
          error.lastName = 'Your lastname must have between 1 and 50 characters.'
        }

        break

      case 'email':
        if (!regexEmail.test(form.email)) error.email = 'Please enter a valid email address.'
        break
