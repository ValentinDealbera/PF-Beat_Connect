...
import { adminGetUsers, adminDeleteUser, setCurrentEditingUser } from '@/redux/slices/admin/users'
  ...
  const rows = usersData.map((item) => ({
    [userVar]: (
      <div className='flex max-w-[50px] items-center gap-4'>
        <Image src={item.image} width={50} height={50} className='aspect-square rounded-full object-cover' alt='user' />

        <div className='flex flex-col'>
          <h3 className='text-base-medium overflow-ellipsis dark:text-white'>{item.username}</h3>
    ...
    <>
      ...
      {elementToDelete && (
        <ModalTables
          ...
          onConfirm={async () => await dispatch(adminDeleteUser(elementToDelete))}
        />
      )}
    </>
  )
