...
export default function AuthorProfile({
  ...
  const { currentAuthor, currentAuthorBeats, loadingcurrentAuthor } =
    useAppSelector((state) => state.beats);

  return (
    <>
      <Hero
        style={{ height: "45vh" }}
        image={currentAuthor?.backImage}
        imageAlt=""
      >
        <div
          id="contenido"
      ...
      <Section subClassName="padding-x-estilo2 padding-y-estilo2 gap-8 flex flex-col">
        {loadingcurrentAuthor ? (
          <Loader />
        ) : (
          <BeatsGrid beats={currentAuthorBeats} />
        )}
      </Section>
    </>
  );
