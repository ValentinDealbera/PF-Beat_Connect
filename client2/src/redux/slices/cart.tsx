...
export const addToCart = (obj:any) => async (_, {dispatch, getState}) => {
  const state = getState().cart;
  const isInCart = state.cart.some((item:any) => item.beat._id === obj.beat._id);
  ...
  if (isInCart === true) {
    ...
    let trad= i18next?.language == "en"? "Successfully added" : "Se agregó al carrito"
    toast.success(trad, toastSuccess);
