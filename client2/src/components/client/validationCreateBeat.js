    ...
    switch (field) {
      case "name":
        if (!regexName.test(form.name)){
          if (i18next?.language == "en") {
            error.name = "The name of your beat must have between one and 50 characters.";
          } else {
            error.name = "El nombre de tu beat debe tener entre uno y 50 caracteres.";
          }
        };
        break;
      ...
      case "audioMP3":
        ...
        if (form.audioMP3.size > MAX_FILE_SIZE){
          if (i18next?.language == "en") {
            error.audioMP3 = "The size of the audio is too large. Maximum allowed size is 10 MB.";
          } else {
            error.audioMP3 = "El tamaño del audio es demasiado grande. El tamaño máximo permitido es de 10 MB.";
          }
        };
        if (!form.audioMP3) {
          if (i18next?.language == "en") {
      ...
      case "audioWAV":
        ...
        if (form.audioWAV.size > MAX_WAV_SIZE){
          if (i18next?.language == "en") {
            error.audioWAV = "The size of the audio is too large. Maximum allowed size is 150 MB.";
          } else {
            error.audioWAV = "El tamaño del audio es demasiado grande. El tamaño máximo permitido es de 150 MB.";
          }
        };
        if (!form.audioWAV) {
          if (i18next?.language == "en") {
